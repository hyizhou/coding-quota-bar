  ...
  return {
    ...
    codex: {
      ...
      details: {
        ...
        quotas: [
          { label: 'quota.codexSessionWindow', labelParams: { n: 5 }, used: 65, total: 100, usageRate: 65, resetAt: new Date(now + 2 * HOUR).toISOString(), limitType: 'codex' },
          { label: 'quota.codexWeeklyWindow', used: 35, total: 100, usageRate: 35, resetAt: new Date(now + 18 * HOUR).toISOString(), limitType: 'codex' },
          { label: 'quota.codexSparkSession', labelParams: { n: 5 }, used: 45, total: 100, usageRate: 45, resetAt: new Date(now + 3 * HOUR).toISOString(), limitType: 'codex-spark' },
          { label: 'quota.codexSparkWeekly', used: 25, total: 100, usageRate: 25, resetAt: new Date(now + 2 * DAY).toISOString(), limitType: 'codex-spark' },
          { label: 'quota.codexExtraLimit', labelParams: { name: 'gpt-5.2-codex' }, used: 30, total: 100, usageRate: 30, resetAt: new Date(now + 5 * HOUR).toISOString(), limitType: 'codex-extra' },
          ...
          { label: 'quota.codexSubscriptionUntil', used: 0, total: 0, usageRate: 0, resetAt: '2026-12-15T00:00:00Z', hideBar: true, limitType: 'codex-subscription' },
          { label: 'quota.codexResetCredits', used: 2, total: 0, usageRate: 0, resetAt: new Date(now + 5 * DAY).toISOString(), hideBar: true, displayUnit: 'count', limitType: 'codex-reset-credits' },
        ],
        codexStats: {
          todayTokens: 86200,
          yesterdayTokens: 143800,
          last30dTokens: 2400000,
          lifetimeTokens: 18600000,
          peakDailyTokens: 512000,
    ...
    stepfun: {
      // 积分型套餐示例：「Credit 用量」总卡（Σresidual/Σtotal 合并口径）+ 订阅桶/加油包桶明细，
      // 含余额与按小时×模型用量历史
      used: 10.33,
      total: 100,
      ...
      details: {
        quotas: [
          // 总卡：Σ剩余 1,614M / Σ总量 1,800M = 剩余 89.67% → 已用 10.33%
          { label: 'quota.stepfunCredits', used: 10.33, total: 100, usageRate: 10.33, resetAt: new Date(now + 30 * DAY).toISOString(), displayUnit: 'percent', limitType: 'stepfun-credits' },
